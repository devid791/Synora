...
import { anthropicUiWorkflow } from "./fixtures/anthropic-ui-workflow";
import { sha256File } from "../src/engine/core-runtime";
test("Anthropic native IPC/UI: signed tool round-trip, cold restart, cancellation, credential deletion and 150% layout", async () => {
  ...
  const provider = suffix
    ? {
        id: `anthropic-native-${suffix}`,
        name: `Owned Claude API ${suffix.slice(-8)}`,
      }
    : undefined;
  ...
  const errors: string[] = [];
  let result: Awaited<ReturnType<typeof anthropicUiWorkflow>>;
  const launch = async () => {
  ...
  try {
    await launch();
    result = await anthropicUiWorkflow({
      fixture: f,
      provider,
      page: () => page,
      snapshot: () =>
      ...
      capture: async (token) => {
        ...
        await mkdir("out/live-evidence/anthropic-ui", { recursive: true });
        ...
        await writeFile(
          `out/live-evidence/anthropic-ui/${platform}-150.png`,
  ...
  await writeFile(
    `out/live-evidence/anthropic-provider-native-${platform}.json`,
    JSON.stringify(
      {
        passed: true,
        scope: process.env.SYNORA_TEST_EXECUTABLE
          ? `Packaged ${platform} UI/IPC, original Core, controlled Anthropic; not public inference or full release qualification`
          : `Native ${platform} development UI/IPC, original Core, controlled Anthropic; not public inference or final installer`,
        ...
        executableSha256: process.env.SYNORA_TEST_EXECUTABLE
          ...
          : null,
        pixels: `out/live-evidence/anthropic-ui/${platform}-150.png`,
