...
test("Mistral adapter with original Core: full tools, actual command/result, cold resume and quiet-stream cancellation", async () => {
  ...
  const create = () =>
    new LiveEngine(
      {
        provider: "synora_mistral",
        executable: f.wrapper,
        stateDirectory: join(f.directory, "synora-mistral"),
        endpoint: f.endpoint,
  ...
  try {
    ...
    expect(
      f.requests.every(
        (v) => v.authorized && v.body.reasoning_effort === "high",
      ),
    ).toBe(true);
    ...
    await writeFile(
      "out/live-evidence/mistral-provider-core.json",
      JSON.stringify(
        {
          scope:
            "Original Core + Synora Mistral provider + controlled Responses; not public Mistral inference",
    ...
    await mkdir("out/live-evidence", { recursive: true });
    await writeFile(
      "out/live-evidence/mistral-provider-core-failure.json",
      JSON.stringify(
        {
