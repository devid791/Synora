...
import type { controlledMistral } from "./mistral-core-upstream";
export async function mistralUiWorkflow(options: {
  page(): Page;
  fixture: Awaited<ReturnType<typeof controlledMistral>>;
  ...
  const providerId = options.provider?.id ?? "mistral-native";
  const providerName = options.provider?.name ?? "Owned Mistral API";
  ...
  await page().getByLabel("Provider adapter").selectOption("mistral");
  ...
  await expect(token()).toContainText("Mistral API key");
  ...
  await nav("Use live Mistral");
  await expect(page().locator(".engine-settings")).toContainText(
    "Current mode: Mistral",
  );
  await nav("Workspace");
