...
if ($project -ne 'C:\Synora_QA_c7a7fb8_20260909_6774e0336697' -or $env:COMPUTERNAME -ne 'AXIOM-WIN-BUILD') {throw 'Unexpected owned QA root'}
...
public static class C7a7WindowsNativeStation {
...
'@
if ([C7a7WindowsNativeStation]::Name() -ine 'WinSta0') {throw 'Requires interactive WinSta0'}
$build=Get-Content -LiteralPath 'out\live-evidence\c7a7-windows-build.json' -Raw | ConvertFrom-Json
if (-not $build.passed -or $build.source -ne 'c7a7fb873dce16e403cb6b127c32ed5fbdbfd683') {throw 'Frozen candidate build is not complete'}
...
$output=Join-Path $project ('out\live-evidence\c7a7-windows-independent-'+$id)
