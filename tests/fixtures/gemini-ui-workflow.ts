...
import type { controlledGemini } from "./gemini-upstream";
export async function geminiUiWorkflow(options: {
  page(): Page;
  fixture: Awaited<ReturnType<typeof controlledGemini>>;
  ...
  const providerId = options.provider?.id ?? "gemini-native";
  const providerName = options.provider?.name ?? "Owned Gemini API";
  ...
  await expect(token()).toContainText("Gemini API key");
  await expect(token()).toContainText("No token saved");
  await expect(token()).toContainText("select Google browser login");
  ...
  await expect(page().getByLabel("Model reasoning effort")).toBeDisabled();
  await nav("Use live Gemini");
  await expect(page().locator(".engine-settings")).toContainText(
    "Current mode: Gemini",
  );
  await nav("Workspace");
