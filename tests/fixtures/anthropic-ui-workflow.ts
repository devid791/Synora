...
import type { controlledAnthropic } from "./anthropic-upstream";
export async function anthropicUiWorkflow(options: {
  page(): Page;
  fixture: Awaited<ReturnType<typeof controlledAnthropic>>;
  ...
  const providerId = options.provider?.id ?? "anthropic-native";
  const providerName = options.provider?.name ?? "Owned Claude API";
  ...
  await page().getByLabel("Provider adapter").selectOption("anthropic");
  ...
  await expect(token()).toContainText("Anthropic API credential");
  await expect(token()).toContainText("No token saved");
  await expect(token()).toContainText("no browser grant is configured");
