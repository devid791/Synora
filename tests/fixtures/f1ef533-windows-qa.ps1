...
if ($project -ne 'C:\Synora_QA_f1ef533_20260910_cacac04fcfd8' -or $env:COMPUTERNAME -ne 'AXIOM-WIN-BUILD') {throw 'Unexpected owned QA root'}
Set-Location $project
$node='C:\Program Files\nodejs\node.exe'
$env:Path='C:\Program Files\nodejs;'+$env:Path
$source='f1ef533fcceb24dc882edc2d96a9bcfc73a2f6c4'
$package='out\production-qa-windows-f1ef533'
$reuse='C:\Synora_QA_ac17c1c_20260910_c05c8a76b463'
$session=[Diagnostics.Process]::GetCurrentProcess().SessionId
$elevated=([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
$receipt=Join-Path $project ('out\live-evidence\f1ef533-windows-'+$Phase.ToLower()+'.json')
...
try {
  if ($Phase -eq 'Build') {
    ...
    Step 'frozen-source-admission' {
      ...
      if ($r.archiveSha256 -ne 'a939d65392bbb573c3623d1d5038f8d7adc965a968a6ecb3f351511d821845b8' -or $r.compiledReceiptSha256 -ne 'fede0e3567a806410e3400a5b088d5812e4816b1ea853ee765801af4afa12a2b') {throw 'Canonical archive/receipt mismatch'}
    ...
    Step 'x64-nsis-and-unpacked' {
      ...
      & 'C:\Program Files\nodejs\npm.cmd' run package:win -- --x64 --config.directories.output=out/production-qa-windows-f1ef533 --publish never
...
public static class F1ef533QaStation {
...
'@
    if ([F1ef533QaStation]::Name() -ine 'WinSta0') {throw 'Interactive station required'}
    $r.station='WinSta0'
    $build=Get-Content -LiteralPath 'out\live-evidence\f1ef533-windows-build.json' -Raw|ConvertFrom-Json
    ...
    foreach ($case in $cases) {
      ...
      & cmd.exe /d /c ('node node_modules/@playwright/test/cli.js test --workers=1 --retries=0 --config '+$case.config+' > out/live-evidence/f1ef533-'+$case.name+'.stdout.log 2> out/live-evidence/f1ef533-'+$case.name+'.stderr.log')
      $status=$LASTEXITCODE
    ...
    $raw=& $node tests/fixtures/package-receipt.mjs out/production-qa-windows-f1ef533 --label=independent-five test-results/desktop.json test-results/ui-quality-desktop.json test-results/core-update-native.json test-results/provider-native.json
