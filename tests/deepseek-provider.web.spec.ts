...
test("DeepSeek shared web UI: actual native tool/result, plain reasoning history, cold resume, cancellation and credential removal", async ({
  page,
}) => {
  ...
  try {
    ...
    const result = await deepseekUiWorkflow({
      fixture: f,
      page: () => page,
      ...
      capture: async () => {
        ...
        await page.screenshot({
          path: "out/live-evidence/deepseek-ui/web-390.png",
          fullPage: true,
        });
      },
    });
    ...
    await writeFile(
      "out/live-evidence/deepseek-provider-web.json",
      JSON.stringify(
        {
          passed: true,
          scope:
            "Shared UI/service and original Core, controlled DeepSeek Responses; not public inference",
