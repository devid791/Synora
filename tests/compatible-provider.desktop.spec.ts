...
import { compatibleUiWorkflow } from "./fixtures/compatible-ui-workflow";
import { sha256File } from "../src/engine/core-runtime";
test("Compatible endpoint native IPC/UI: native tool round-trip, cold restart, cancellation, credential deletion and 150% layout", async () => {
  ...
  const provider = suffix
    ? {
        id: `compatible-native-${suffix}`,
        name: `Owned Compatible endpoint API ${suffix.slice(-8)}`,
      }
    : undefined;
  ...
  const errors: string[] = [];
  let result: Awaited<ReturnType<typeof compatibleUiWorkflow>>;
  const launch = async () => {
  ...
  try {
    await launch();
    result = await compatibleUiWorkflow({
      fixture: f,
      provider,
      page: () => page,
      snapshot: () =>
      ...
      capture: async (token) => {
        ...
        // capturePage uses the native compositor, not the earlier CDP layout.
        await page.waitForTimeout(150);
        expect(
          await page.evaluate(
        ...
        await mkdir("out/live-evidence/compatible-ui", { recursive: true });
        ...
        await writeFile(
          `out/live-evidence/compatible-ui/${platform}-150.png`,
  ...
  await writeFile(
    `out/live-evidence/compatible-provider-native-${platform}.json`,
    JSON.stringify(
      {
        passed: true,
        scope: process.env.SYNORA_TEST_EXECUTABLE
          ? `Packaged ${platform} UI/IPC, original Core, controlled Compatible endpoint; not public inference or full release qualification`
          : `Native ${platform} development UI/IPC, original Core, controlled Compatible endpoint; not public inference or final installer`,
        ...
        executableSha256: process.env.SYNORA_TEST_EXECUTABLE
          ...
          : null,
        pixels: `out/live-evidence/compatible-ui/${platform}-150.png`,
