...
test("xAI native IPC/UI: saved key, real Core command, restart identity, cancellation and 150% layout", async () => {
  const f = await controlledOpenAi({ xai: true });
  const providerId =
    process.platform === "win32" ? `native-xai-${randomUUID()}` : "native-xai";
  const providerName =
    process.platform === "win32"
      ? `Native xAI ${providerId.slice(-8)}`
      : "Native xAI";
  ...
  try {
    ...
    await mkdir("test-results/xai-provider-native", { recursive: true });
    ...
    await writeFile(
      "test-results/xai-provider-native/settings-150.png",
    ...
    await writeFile(
      `out/live-evidence/xai-provider-native-${process.platform === "win32" ? "windows" : "linux"}.json`,
      JSON.stringify(
        {
          passed: true,
          scope: `${process.platform} native UI/IPC, original Core, controlled xAI Responses; not public account inference or final installers`,
