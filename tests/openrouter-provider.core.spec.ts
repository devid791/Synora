...
test("OpenRouter non-reasoning model: original Core does not invent an effort override", async () => {
  expect(Object.keys(networkInterfaces()).sort()).toEqual(["lo"]);
  const f = await controlledOpenAi({
    openrouter: true,
    openrouterNoReasoning: true,
  });
  const engine = new LiveEngine(
    {
      provider: "synora_openrouter",
      executable: f.wrapper,
      stateDirectory: join(f.directory, "synora-openrouter"),
      endpoint: f.endpoint,
      model: "fixture/router-model",
      authorization: async () => f.key,
      context: () => ({ cwd: f.workspace, profile: "", context: null }),
      bind: () => {},
      sink: () => {},
    },
    prepareOpenRouterProcess,
  ...
  try {
    const models = await openRouterModels(f.endpoint, f.key);
    expect(models[0].reasoning_efforts).toEqual([]);
    expect(models[0].default_reasoning_effort).toBeUndefined();
    await engine.start(
      "owned-no-reasoning",
    ...
    expect(
      f.requests.every((v) => v.authorized && v.body.reasoning?.effort == null),
    ).toBe(true);
    expect(f.errors).toEqual([]);
    await mkdir("out/live-evidence", { recursive: true });
    await writeFile(
      "out/live-evidence/openrouter-no-reasoning-core.json",
      JSON.stringify(
        {
          scope:
            "Original Core non-reasoning model + controlled OpenRouter upstream; not public inference",
          passed: true,
...
test("OpenRouter adapter with original Core: full tools, actual command/result, cold resume and quiet-stream cancellation", async () => {
  expect(Object.keys(networkInterfaces()).sort()).toEqual(["lo"]);
  ...
  try {
    const catalog = await openRouterModels(f.endpoint, f.key);
    expect(catalog[0].context_window).toBe(131072);
    expect(catalog[1].unavailableReason).toBeTruthy();
    engine = create();
    await engine.start(
      "owned-openrouter-conversation",
      "Read provider-check.txt and confirm its contents",
      "text",
    );
    await expect
    ...
    await mkdir("out/live-evidence", { recursive: true });
    await writeFile(
      "out/live-evidence/openrouter-provider-core.json",
      JSON.stringify(
        {
          scope:
            "Original Core + Synora OpenRouter provider + controlled Responses; not public OpenRouter inference",
          coreSha256: await sha256File(f.core),
          passed: true,
          first,
          binding,
    ...
    await mkdir("out/live-evidence", { recursive: true });
    await writeFile(
      "out/live-evidence/openrouter-provider-core-failure.json",
      JSON.stringify(
        {
          snapshot: engine?.snapshot(),
          requests: f.requests,
