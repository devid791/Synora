...
test("Mistral native IPC/UI: native tool round-trip, cold restart, cancellation, credential deletion and 150% layout", async () => {
  ...
  const provider = suffix
    ? {
        id: `mistral-native-${suffix}`,
        name: `Owned Mistral API ${suffix.slice(-8)}`,
      }
    : undefined;
  ...
  const errors: string[] = [];
  let result: Awaited<ReturnType<typeof mistralUiWorkflow>>;
  const launch = async () => {
  ...
  try {
    await launch();
    result = await mistralUiWorkflow({
      fixture: f,
      provider,
      page: () => page,
      snapshot: () =>
      ...
      capture: async (token) => {
        ...
        await writeFile(
          `out/live-evidence/mistral-ui/${platform}-150.png`,
  ...
  await writeFile(
    `out/live-evidence/mistral-provider-native-${platform}.json`,
    JSON.stringify(
      {
        passed: true,
        scope: process.env.SYNORA_TEST_EXECUTABLE
          ? `Packaged ${platform} UI/IPC, original Core, controlled Mistral; not public inference or full release qualification`
          : `Native ${platform} development UI/IPC, original Core, controlled Mistral; not public inference or final installer`,
        ...
        executableSha256: process.env.SYNORA_TEST_EXECUTABLE
          ...
          : null,
        pixels: `out/live-evidence/mistral-ui/${platform}-150.png`,
