...
import { deepseekUiWorkflow } from "./fixtures/deepseek-ui-workflow";
import { sha256File } from "../src/engine/core-runtime";
test("DeepSeek native IPC/UI: native tool round-trip, cold restart, cancellation, credential deletion and 150% layout", async () => {
  ...
  const provider = suffix
    ? {
        id: `deepseek-native-${suffix}`,
        name: `Owned DeepSeek API ${suffix.slice(-8)}`,
      }
    : undefined;
  ...
  const f = await controlledOpenAi({ deepseek: true });
  let app: ElectronApplication | undefined, page: Page;
  const errors: string[] = [];
  let result: Awaited<ReturnType<typeof deepseekUiWorkflow>>;
  const launch = async () => {
  ...
  try {
    await launch();
    result = await deepseekUiWorkflow({
      fixture: f,
      provider,
      page: () => page,
      snapshot: () =>
      ...
      capture: async (token) => {
        ...
        await mkdir("out/live-evidence/deepseek-ui", { recursive: true });
        ...
        await writeFile(
          `out/live-evidence/deepseek-ui/${platform}-150.png`,
  ...
  await writeFile(
    `out/live-evidence/deepseek-provider-native-${platform}.json`,
    JSON.stringify(
      {
        passed: true,
        scope: process.env.SYNORA_TEST_EXECUTABLE
          ? `Packaged ${platform} UI/IPC, original Core, controlled DeepSeek; not public inference or full release qualification`
          : `Native ${platform} development UI/IPC, original Core, controlled DeepSeek; not public inference or final installer`,
        ...
        executableSha256: process.env.SYNORA_TEST_EXECUTABLE
          ...
          : null,
        pixels: `out/live-evidence/deepseek-ui/${platform}-150.png`,
