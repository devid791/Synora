...
test("Anthropic shared web UI: actual native tool/result, signed history, cold resume, cancellation and credential removal", async ({
  page,
}) => {
  ...
  try {
    ...
    const result = await anthropicUiWorkflow({
      fixture: f,
      page: () => page,
      ...
      capture: async () => {
        ...
        await page.screenshot({
          path: "out/live-evidence/anthropic-ui/web-390.png",
          fullPage: true,
        });
      },
    });
    ...
    await writeFile(
      "out/live-evidence/anthropic-provider-web.json",
      JSON.stringify(
        {
          passed: true,
          scope:
            "Shared UI/service and original Core, controlled native Anthropic; not public inference",
