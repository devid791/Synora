...
test("DeepSeek adapter with original Core: full tools, actual command/result, cold resume and quiet-stream cancellation", async () => {
  ...
  const create = () =>
    new LiveEngine(
      {
        provider: "synora_deepseek",
        executable: f.wrapper,
        stateDirectory: join(f.directory, "synora-deepseek"),
        endpoint: f.endpoint,
  ...
  try {
    const catalog = await deepSeekModels(f.endpoint, f.key);
    expect(catalog[0].context_window).toBe(1000000);
    expect(catalog[1].unavailableReason).toBeTruthy();
    engine = create();
    await engine.start(
      "owned-deepseek-conversation",
    ...
    await writeFile(
      "out/live-evidence/deepseek-provider-core.json",
      JSON.stringify(
        {
          scope:
            "Original Core + Synora DeepSeek provider + controlled Responses; not public DeepSeek inference",
    ...
    await mkdir("out/live-evidence", { recursive: true });
    await writeFile(
      "out/live-evidence/deepseek-provider-core-failure.json",
      JSON.stringify(
        {
