// Real shared UI/service + original Core + a controlled xAI endpoint.
// No real xAI key or public paid inference; only loopback is available.
...
test("xAI shared UI: endpoint/key/catalog, real command, cold resume, cancellation and key removal", async ({
  page,
}) => {
  ...
  try {
    ...
    await page.getByLabel("Configuration ID").fill("xai");
    await page.getByLabel("Configuration name").fill("Owned xAI");
    await page.getByLabel("Provider adapter").selectOption("xai");
    ...
    const auth = page.getByRole("region", {
      name: "Bearer token for Owned xAI",
    });
    await expect(auth).toContainText("xAI API key");
    await expect(auth).toContainText("No token saved");
    expect((await server.service.api.engineModels("xai")).ok).toBe(false);
    expect(f.requests).toEqual([]);
    await auth.getByLabel("Bearer token for Owned xAI").fill(f.key);
    ...
    await expect(auth.getByLabel("Bearer token for Owned xAI")).toHaveValue("");
    await nav("Settings");
    await page.getByLabel("Engine provider").selectOption("xai");
    await nav("Read live model catalog");
    await expect(page.getByLabel("Engine model")).toHaveValue("grok-4.6");
    await expect(
      page.locator('option[value="unqualified-fixture-model"]'),
    ...
    expect(
      (
        await server.service.api.engineConfigure({
          mode: "live",
          providerId: "xai",
          model: "unqualified-fixture-model",
        })
      ).ok,
    ).toBe(false);
    ...
    expect((await server.service.api.providerCredentialDelete("xai")).ok).toBe(
      false,
    );
    ...
    for (const width of [1440, 390]) {
      ...
      await page.screenshot({
        path: `test-results/xai-provider-web/workspace-${width}.png`,
        fullPage: true,
      });
    }
    await nav("Models & accounts");
    ...
    await writeFile(
      "out/live-evidence/xai-provider-web.json",
      JSON.stringify(
        {
          passed: true,
          scope:
            "Real shared UI and original Core against controlled xAI Responses, not public paid inference",
    ...
    await writeFile(
      join(f.directory, "xai-ui-diagnostics.json"),
      JSON.stringify(
        {
          errors,
