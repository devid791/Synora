...
test("Compatible endpoint shared web UI: actual native tool/result, full Responses history, cold resume, cancellation and credential removal", async ({
  page,
}) => {
  ...
  try {
    ...
    const result = await compatibleUiWorkflow({
      fixture: f,
      page: () => page,
      ...
      capture: async () => {
        ...
        await page.screenshot({
          path: "out/live-evidence/compatible-ui/web-390.png",
          fullPage: true,
        });
      },
    });
    ...
    await writeFile(
      "out/live-evidence/compatible-provider-web.json",
      JSON.stringify(
        {
          passed: true,
          scope:
            "Shared UI/service and original Core, controlled Compatible endpoint Responses; not public inference",
